...
/**
 * TODO refactor
 * Find modules affected by loader.
 * @param {NormalModule[]} modules
 * @return {NormalModule[]}
 */
function getAffectedModules(modules) {
  return modules.filter((module) => {
    const resource = module.resource;
    const rule = module.loaders.find(l => l.loader === loaderPath);
    ...
    const extract = (rule.options && rule.options.extract) || (issuer && defaults.extractIssuerRegExp.test(issuer));
    return extract;
  });
}

exports.getAffectedModules = getAffectedModules;
...
function aggregate(symbols, compilation) {
  ...
  const modules = getAffectedModules(compilation.modules);
