...
module.exports = function loader(content) {
  ...
  const { resourcePath, context, loaderIndex } = loaderContext;
  // webpack 1 compat
  ...
  if (!content.includes('<svg')) {
    throw new Exceptions.InvalidSvg(content);
  }

  const configObj = { context: loaderContext };

  configObj.config = getOptions(loaderContext);
  configObj.target = context.target || loaderContext.target || (context.options && context.options.target);
